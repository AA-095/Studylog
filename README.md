...
## クライアントサイドキャッシング
- クライアント側でキャッシュを蓄える方式をクライアントサイドキャッシングと呼ぶ。クライアントサイドキャッシングの主な手段として、ブラウザーのキャッシュ機能がある。プロキシサーバーを使って多人数で利用するキャッシュを共有型キャッシュ、ブラウザーのように特定のユーザーのみ利用するキャッシュを非共有型キャッシュと呼ぶ。

## サーバーサイドキャッシング
- サーバー側でキャッシュを蓄える方式をサーバーサイドキャッシングと呼ぶ。サーバーサイドキャッシングでは、頻繁に使用されるコンテンツをキャッシュとし、クライアントからの要求にはキャッシュで応答する。サーバーサイドキャッシングでは、Webサーバー内にキャッシュ領域を持つ方式のほか、プロキシサーバーを使った方式もあり、プロキシサーバーがブラウザからのリクエストに代理応答することで、Webサーバーのレスポンスを改善する。
...
- ロードバランサーによる負荷分散にはいくつかの方式がある。ラウンドロビン方式では、単純に順番通りに各サーバーに割り振りする。優先順位方式では、各オリジンサーバーに優先順位を設定し、優先順位の高いサーバーからリクエストを割り振る。重み付け方式では、割り当てるオリジンサーバーの頻度を重みで変えることができる。オリジンサーバーの性能に差がある場合、低スペックなサーバーへの割り振りは減らし、高スペックなサーバーへの割り振りは増やすなどの対応が可能である。
