...
- HTMLはハイパーテキストを記述する際に使用する言語である。マークアップ言語とは、各文章に対する構造や見た目などを指定するための言語である。

## HTMLタグ
- HTMLでは、各文章に対する構造や見た目などを指定する際はHTMLタグを用いる。対象文字列の前に置かれるタグを開始タグ、後ろのタグを終了タグと呼ぶ。「＜br＞」や「＜img＞」等はタグ単体で意味を持っており、対象文字列を必要としないHTMLタグには終了タグは存在していない。

## HTMLの基本構造
- HTMLドキュメントは、大きく文書型宣言、ヘッダー、ボディで構成されている。HTMLドキュメントの先頭には、その文書がどのような文書型定義に基づいて記述されているかを「＜!DOCTYPE…＞」を使って宣言する。
