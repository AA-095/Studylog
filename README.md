...
- サーバーから受け取ったデータをもとにクラフィック化して画面に表示する機能を,レンダリングと呼ぶ。

# プロトコルとは
ネットワーク通信において、送られる側と受け取る側で事前にルールを取り決めておく必要がある。この取り決めをプロトコルと呼ぶ。

## プロトコルの標準化
世界中のWebサーバーにアクセスし、表示することができるように標準化という作業が必要になる。標準化とは何もしなければ秩序が取れないものに標準となる規定を取り決め、互換性や品質を確保して技術の普及を図ることである。
...
- IPパケットを送信する場合は、送信先のIPアドレスを指定し、特定のコンピューターにパケットを送信する。その際に送信元と送信先が同じLAN内にあれば直接通信が行えるが、他のネットワークにある場合は、ルーターという危機を経由する。ルーターはルーティングテーブルという経路情報によって、IPアドレスから送信先のコンピューターがネットワークに接続されているか知ることができる。
- 自分のネットワーク内に送信先コンピューターがない場合は、デフォルトゲートウェイと呼ばれるルーターに送られる。
...
## TCPの信頼性を挙げる仕組みとUDP
- TCPの信頼性を高める機能の一つに3ウェイハンドシェイクがある。3ウェイハンドシェイクとは、送信前にパケットを受け取る準備ができているか確認するやり取りのことである。また、送信パケットにはシーケンスと呼ばれる通し番号が付与されており、その順番通りにパケットを受信しているかを確認したり、届いていないパケットがないかを検知できるようになっている。
...
- IPv4アドレスは、ネットワーク部とホスト部に分けられる。例えば「192.168.1」をネットワーク部とした場合、2進数の残り8ビットを0で埋めたものをネットワークアドレスとして利用する。
...
## IPv6アドレス
- IPv6アドレスは、128ビットのビット列を先頭から16ビットごとに8つに区切って、16進数に変換してコロンで連結する。
...
- プロトコルネゴシエーション
    ...
    - アップグレード方式では、HTTP/2対応ブラウザーが最初はHTTP/1.1での手順でWebサーバーに接続を行い、WebサーバーがHTTP/2に対応していた場合はステータスコードとして「101」を返し、HTTP/2コネクションにアップグレードする。
...
## URIとURL
- URLはURIと呼ばれる識別子のサブセットである。URIはURLにURNの概念を加えたものの総称である。URLはサイトがどこのあり、どのリソースをダウンロードすればいいのか場所や位置を示すのに対し、URNは識別に用いられる。

## URLの構文
- URLにおいて一番左に位置するものはスキームと呼ばれ、通信方式であるプロトコルを表す。
- 「www.example.jp」のように一般的にはホームページのアドレスとしてイメージされる部分をオーソリティと呼ぶ。オーソリティは「//」ではじまり、ホスト名、サービスポート番号などを含む。またユーザー認証が必要なサービスでは、「ユーザー名：パスワード＠」の形式でクレデンシャルを入れてブックマークしておくとアクセスのたびに入力しなくて済む。
- パスはサーバー内のファイルの格納場所など、特定のリソースを識別するために使用する。「www.example.jp/path/index.html」である場合、サーバー内のpathディレクトリの中にあるindex.htmlファイルを指定している。パスを省略して特定のファイルを指定しない場合は、デフォルトドキュメントが返される。パスの後にも必要に応じてクエリやフラグメントなどが指定可能である。フラグメントでは同じページ内の特定の場所を指定するアンカーに使用する。

## URLに使える文字列・文字長
- URLにはアルファベット・数字・記号等の非予約文字を使うことができるが、「：」や「＠」などの文字は区切り文字として、「％」はエスケープ文字として予約されているので使えない。通常の文字列として使用したい場合は、パーセントエンコーディングでエスケープ処理を行うか、Punycodeを用いる必要がある。また、URLの最大文字数は規定されていない。

## 絶対URL・相対URL
- 情報のありかを示す場合にはパスを使用する。PCの中に保存されたファイルの場所をファイルパスで、実行するコマンドをコマンドパスを使って説明する。絶対パスは、フルパスと呼ばれ、/（ルート）と呼ばれる階層構造の根本から目的までのパスを表している。ファイルとフォルダ―の区切りに使う文字として一般には「/」を用いるが、Windowsでは「￥」を用いる。相対パスは、ユーザーが現在作業しているフォルダーから目的までのパスを表している。ユーザーが現在作業しているフォルダーをカレントディレクトリと呼び、「.」で表す。
- URLのパス部分に絶対パスを含んだものを絶対URL、相対パスを含んだものを相対URLと呼ぶ。相対URLはベースURLと呼ばれる基準点からの相対的な位置を表す。

## パーセントエンコーディング・Punycode
- ％エンコーディングは、URLのパス部分で使用できない文字を扱えるようにするためのエスケープ処理である。一般的にはURLエンコードとも呼ばれている。
- 最近は国際化ドメイン名が普及している。ドメイン名は英語表記のみになっているため、Punycodeに変換する必要がある。

## 短縮URL・ワンタイムURL
- 長い文字列の絶対URLを入力しやすいように短いURLに置き換えたサービスが短縮URLである。短縮URLはWebサーバーのリダイレクト機能を利用して、短縮したURLにアクセスがあった場合は、正規のサーバーに転送する仕組みである。
- 短縮URLにリダイレクトできる期間や回数を設定することで、制限付きのURLとしても利用できる。たとえばリダイレクトを1回のみに設定すれば、ワンタイムURLになる。

## URLによるSEO対策
- 検索サイトの上位にランキングされるための対策をSEO（検索エンジン最適化）と呼ぶ。URL中のパスを簡潔にすることでもSEO対策が可能である。会員サイトでは、URLにセッションIDと呼ばれる識別情報やクエリーが含まれる。URLが長くなるとSEO対策には不利になる。検索サイトのランキングでは、被リンクアカウントと呼ばれる値で他のWebページからどれだけリンクされているかを計算しており、これが多いほど検索ランキングで上位になる傾向がある。

# サーバーの役割と機能
## Webシステムの高速化・大規模化
- Webサイトを構築する際のルールとして6秒ルールがある。これは6秒以内に表示を完了させないと、ユーザーを取り逃がしてしまうというものである。最近では、スマートフォンなどのモバイル端末での利用が増えたことによって、3秒ルールというさらに厳しいルールもできている。
- スケールアップ（垂直スケール）とは、サーバーのCPUやメモリーなどを増強してサーバー単体の処理能力を向上することで、Webサーバーにかかる負荷を減らして大量のリクエストを裁けるようにすることである。しかし、性能を高くしてもサーバー1台の場合は耐障害性を高めることはできない。
- スケールアウト（水平スケール）とは、複数のサーバーをシステム化して、システム全体の性能を向上させる手法である。複数サーバーのシステム化による処理能力の向上は、耐障害性、拡張性、高レスポンスなどの面でも有効である。

## プロキシサーバー
- Webシステムでは、プロキシと呼ばれる機能を持つサーバーによってブラウザーとWebサーバー間のやり取りを中継することができる。プロキシは代理という意味である。これらの中継の過程でWebサーバーからブラウザーでのデータをプロキシサーバーが一時的に蓄えたり、URLなどの条件によってリクエスト先のサーバーを変更することで、Webアクセスの高速化や大規模化が可能になる。
- オリジナルのリソースをもったWebサーバーをオリジンサーバーと呼ぶ。プロキシサーバーを経由した場合はレスポンスメッセージにViaヘッダーが追加されるため、どのプロキシサーバーを中継したか分かる。
- ブラウザー側にプロキシサーバーを導入するメリットは、Webアクセスをプロキシサーバーに集中させることができ、ネットワークを効率的に利用できることである。プロキシサーバーでWebサーバーとのやり取りをチェックし、危険であると判断した場合は通信を遮断するという、WAFのような機能を果たす。またWebコンテンツのウイルスチェックやスキャン、WebサーバーのURLチェックなどを行うことも可能である。さらにレスポンスの高速化というメリットもある。プロキシサーバーがWebアクセスを中継する際、オリジンサーバーのリソースをキャッシュして、次に同じリクエストが発生した際は、このキャッシュデータを再利用してレスポンスの高速化が可能になる。この機能をフォワードプロキシという。
- オリジンサーバーに処理を分散させることをロードバランシングと呼び、リバースプロキシ機能が利用される。リバースプロキシ機能では、ブラウザからのリクエストをいったん中継してからバックエンドのWebサーバーへ振り分ける。複数のWebサーバーに処理を分散させることが可能なほか、Webサーバーの構成を外部から隠ぺいするといった効果もある。バックエンドのオリジンサーバーへの振り分けには、URLマッピングという手法が用いられる。特定のURLにマッチした場合は、指定したオリジンサーバーにリクエストを転送するという仕組みである。

## クライアントサイドキャッシング
- クライアント側でキャッシュを蓄える方式を、クライアントサイドキャッシングと呼ぶ。クライアントサイドキャッシングの主な手段として、ブラウザーのキャッシュ機能がある。プロキシサーバーを使って多人数で利用するキャッシュを共有型キャッシュ、ブラウザーのように特定のユーザーのみ利用するキャッシュを非共有型キャッシュと呼ぶ。

## サーバーサイドキャッシング
- サーバー側でキャッシュを蓄える方式を、サーバーサイドキャッシングと呼ぶ。サーバーサイドキャッシングでは、頻繁に使用されるコンテンツをキャッシュとし、クライアントからの要求にはキャッシュで応答する。サーバーサイドキャッシングでは、Webサーバー内にキャッシュ領域を持つ方式のほか、プロキシサーバーを使った方式もあり、プロキシサーバーがブラウザからのリクエストに代理応答することで、Webサーバーのレスポンスを改善する。
- プロキシサーバーには2種類あり、Webサーバーのフロントエンドにプロキシサーバーを置く方式をリバースプロキシ（Webアクセラレーター）、事業所や家庭に設置して、ブラウザーの機能を補う方式を、フォワードプロキシと呼ぶ。

## ロードバランサー
- サーバーを増強する方法を、スケールアップと呼ぶ。コストをかければ処理能力は向上するが、処理能力にも限界があるため、かけたコストと性能の向上は比例しない。一方、サーバーの台数を増やす方法をスケールアウトと呼ぶ。サーバー台数を二倍に増やせば、性能も比例して向上するため、費用対効果が高く、スケーラビリティに優れている。
- スケーラビリティや耐障害性で有利なスケールアウトだが、クライアントからのリクエストを複数のサーバーに分散させるには、ロードバランサーが必要になる。
- ロードバランサーによる負荷分散にはいくつかの方式がある。ラウンドロビン方式では、単純に順番通りに各サーバーに割り振る。優先順位方式では、各オリジンサーバーに優先順位を設定し、優先順位の高いサーバーからリクエストを割り振る。重み付け方式では、割り当てるオリジンサーバーの頻度を重みで変えることができる。オリジンサーバーの性能に差がある場合、低スペックなサーバーへの割り振りは減らし、高スペックなサーバーへの割り振りは増やすなどの対応が可能である。

## より高度な負荷分散
- ラウンドロビン方式はあらかじめ設定した通りに動作する静的なものでしたが、状況に応じて動的に動作する分散方式もある。最速応答時間方式では、応答が最も早いバックエンドサーバーを優先し、最小コネクション方式や最小トラフィック方式では、接続中のコネクション数やトラフィック量が最も少ないサーバーを優先して割り振る。動的な負荷分散では、ヘルスチェック機能で各オリジンサーバーをモニタリングし、モニターデータをロードバランサー内部に保持する必要がある。
- コンテンツスイッチング方式（L7スイッチ）では、HTTPヘッダーやリクエストメッセージの中身によって割り振りを変更することができる。

## CDN
- CDNとは、世界中のネットワークにWebサーバーを分散配置して、どこからアクセスしても、Webコンテンツを効率的かつ迅速に配信できるようにしたネットワークである。CDN事業者は、SSL/TLS暗号化通信のアクセラレーション、ネットワークやアプリケーションの最適化、コンテンツの圧縮転送、DDosやDoS等の攻撃に対する防御、アプリケーションレベルでのファイアーウォール、地理的に離れたデータセンター間の広域負荷分散など幅広い機能を提供している。
- 世界中に張り巡らされたCDN網は、各所にキャッシュサーバーを配置している。Webコンテンツをキャッシュサーバーにコピーしておくことで、世界中どこからでも低遅延でWebコンテンツへのアクセスを可能にする。これを実現するには、Webコンテンツのオリジンサーバーとキャッシュサーバー間で常に同期をとる必要がある。

## 仮想化技術
- 仮想化技術を使うと、1台のコンピューター上で複数のコンピューターを同時に起動することができる。現実のコンピューターを物理マシンと呼ぶのに対し、仮想的なコンピューターを仮想マシンと呼ぶ。また仮想マシンを稼働させる物理マシン上のOSをホストOS、仮想マシン上で動作しているOSをゲストOSと呼んで区別する。
- 仮想化には大きく分けて2つの型がある。ハードウェアを丸ごとエミュレートするハイパーバイザー型と、アプリケーションの実行環境だけをエミュレートしたコンテナ型である。
- クラウドにはハードウェアリソースやインフラを提供する形態のIaaS、アプリケーションのためのプラットフォームを提供するPaaS、そしてサービスやアプリケーションを提供する形態のSaaSなどの種類がある。

## サーバーレスアーキテクチャ
- 仮想化を進めて、サーバー管理でさえも必要としないようにしたしくみがサーバーレスアーキテクチャである。サーバーレスでは処理の実行条件となるトリガーだけ設定しておけば、あとはすべてプラットフォームに任せることができる。

## コンテナ型仮想化技術
- コンテナ型の特徴は、アプリケーションの実行に必要なプロセスやリソースを切り離して、他のプロセスから独立して実行できる点にある。コンテナ型の実行単位であるコンテナと比較すると、コンテナの方が小さくなる。コンテナは、小さなサービスをAPIによって連携させるアーキテクチャのマイクロサービスの普及とともに、様々なシステムで活用されている。

# Webコンテンツの種類
## ハイパーリンクとHTML
- ハイパーテキストとは、複数のドキュメントの関連付けを可能にしたドキュメントのことである。ハイパーリンクによってWebサイトのリンクをたどることが可能になり、次々に関連する文章を表示することが可能になる。
- HTMLは、ハイパーテキストを記述する際に使用する言語である。マークアップ言語とは、各文章に対する構造や見た目などを指定するための言語である。

## HTMLタグ
- HTMLでは、各文章に対する構造や見た目などを指定する際はHTMLタグを用いる。対象文字列の前に置かれるタグを開始タグ、後ろのタグを終了タグと呼ぶ。「＜br＞」や「＜img＞」等はタグ単体で意味を持っており、対象文字列を必要としないHTMLタグに終了タグは存在していない。

## HTMLの基本構造
- HTMLドキュメントは大きく、文書型宣言、ヘッダー、ボディで構成されている。HTMLドキュメントの先頭には、その文書がどのような文書型定義に基づいて記述されているかを「＜!DOCTYPE…＞」を使って宣言する。
- ヘッダーには、ページタイトル、使用言語、キャッシュの有効期限、検索サイトのためのキーワード、サイトの説明など、HTMLドキュメントに対するメタデータを挿入する。
- ボディにはさまざまなHTMLタグを使って、Webページの本文を記述する。ボディを構成する要素をブロックレベル要素とインライン要素に分けることができる。

## HTMLの互換性
- Webページの画面描画は、ブラウザーのレンダリングエンジンによって行われる。ブラウザーが文書型宣言に合わせてモードを切り替えることをDOCTYPEスイッチと呼ぶ。HTMLブラウザーのレンダリングモードを変えるには、文書型宣言を使用する。具体的には、HTMLの冒頭に「＜!DOCTYPE…＞」を使って宣言する。ブラウザーのレンダリングモードは、標準モードと互換モードの2種類がある。

## CSS
- CSSは、一般的にはスタイルシートと呼ばれている。CSSファイルを作成する際、ファイル名に拡張子の「.css」をつけることでCSSファイルとして認識される。タグごとにスタールを設定し、その単位をルールセットと呼ぶ。各ルールセットは「{}中括弧」で囲み、その中で「プロパティ：プロパティ値；」のように宣言する。

## 静的コンテンツ・動的コンテンツ
- リクエストや内容に影響されず、常に同じ内容になるコンテンツを静的コンテンツと呼ぶ。リクエストの内容に応じて、HTMLドキュメントなどのコンテンツを自動的に生成し、それをクライアントに送信する。このようなコンテンツを動的コンテンツと呼ぶ。
- 動的コンテンツには、Webサーバー側でコンテンツを作成するサーバーサイドと、ブラウザー側で作成するクライアントサイドの二種類がある。動的コンテンツにはサーバーにより大きな負荷がかかるため、動的コンテンツを作成する専用のWebアプリケーションサーバーを用いるのが一般的である。
