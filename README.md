...
- オリジンサーバーに処理を分散させることをロードバランシングと呼び、リバースプロキシ機能が利用される。リバースプロキシ機能ではブラウザからのリクエストをいったん中継してからバックエンドのWebサーバーへ振り分ける。複数のWebサーバーに処理を分散させることが可能なほか、Webサーバーの構成を外部から隠ぺいするといった効果もある。バックエンドのオリジンサーバーへの振り分けには、URLマッピングという手法が用いられる。特定のURLにマッチした場合は、指定したオリジンサーバーにリクエストを転送するという仕組みである。
